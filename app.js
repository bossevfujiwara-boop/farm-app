...
function renderWorkProcess(field) {
  ...
  document.getElementById('work-process-list').innerHTML = groups.length ? groups.map(group => `<div class="work-pole-group"><div class="work-pole-header"><strong>${group.range || 'ポール範囲未設定'}</strong><span>${group.area || '面積未設定'}</span></div><div class="work-pole-crop">${group.crop || '作物未設定'}${group.variety ? ` ${group.variety}` : ''}</div><ul class="work-checklist">${group.tasks.map(task => `<li class="${task.date ? 'done' : ''}"><span>${task.name}</span><span>${task.date ? formatDate(task.date) : '未定'}</span></li>`).join('')}</ul>${group.memo ? `<p class="work-memo">${group.memo}</p>` : ''}<div class="work-temp-row"><span>積算温度：${group.accumulatedTemp || '未設定'}</span><span>適正生育温度：${group.idealTemp || '未設定'}</span></div></div>`).join('') : '<div class="column-empty">作業工程データ未登録<br><small>Excelの「2026-27作業詳細」シートを読み込むと表示されます。</small></div>';
}
