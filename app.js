...
function extractWorkbookPoleRow(row) { const cells = row.map(cell => String(cell ?? '').trim()).filter(Boolean); const poleCell = cells.find(cell => /ポール|pole/i.test(cell) && /\d+/.test(cell)) || cells.find(cell => /^\d+$/.test(cell)); if (!poleCell) return null; const poleNumber = poleCell.match(/\d+/)?.[0]; if (!poleNumber) return null; const length = cells.find(cell => /\d+(?:\.\d+)?\s*m(?:\s|$)/i.test(cell)) || ''; const dateMatches = cells.join(' ').match(/(?:20\d{2}[\/\-]\d{1,2}[\/\-]\d{1,2}|\d{1,2}[\/\-]\d{1,2}|\d{1,2}月\d{1,2}日)/g) || []; const count = cells.find(cell => /\d+\s*(?:本|株)/.test(cell)) || ''; const content = cells.filter(cell => cell !== poleCell && cell !== length && !/^20\d{2}[\/\-]\d{1,2}[\/\-]\d{1,2}$/.test(cell) && !/^\d+(?:\.\d+)?\s*m$/i.test(cell) && !/^(?:列|ポール|長さ|本数|株数|株間|畝|マルチ|作業)/.test(cell)).join(' / '); return { pole: `ポール↑${poleNumber}`, length, crop: content || '〃', planting: dateMatches[0] ? normalizeDateValue(dateMatches[0]) : '〃', count, spacing: cells.find(cell => /株間|cm/i.test(cell)) || '', bed: cells.find(cell => /畝/.test(cell)) || '', mulch: cells.find(cell => /マルチ/.test(cell)) || '', work: cells.find(cell => /作業|定植|誘引|除草|収穫/.test(cell)) || '' }; }
