...
function workbookFieldIds(sheetName) { const match = String(sheetName).match(/^2026-27詳細地図(.+)/i); if (!match) return []; return match[1].split(/[.・、,\s]+/).map(normalizeFieldId).map(id => id === 'g' ? 'gout' : id).filter(id => ['a', 'b', 'c', 'd', 'e1', 'e2', 'f', 'gout', 'g1', 'g2', 'g3', 'g4', 'h', 'i', 'j', 'k', 'l1', 'l2', 'm', 'n', 'o'].includes(id)); }
