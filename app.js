...
function loadFields() { try { const stored = JSON.parse(localStorage.getItem('farmnote-fields-v8-2026-27')); return applyLatestOverrides(stored && stored.length >= defaultFields.length ? stored.map(field => ({ ...field, id: normalizeFieldId(field.id) })) : structuredClone(defaultFields)); } catch { return applyLatestOverrides(structuredClone(defaultFields)); } }
function saveFields() { localStorage.setItem('farmnote-fields-v8-2026-27', JSON.stringify(fields)); document.getElementById('last-updated').textContent = '今 保存済み'; }
...
function extractWorkbookPoleRow(row) { const cells = row.map(cell => String(cell ?? '').trim()).filter(Boolean); const poleCell = cells.find(cell => /ポール|pole/i.test(cell) && /\d+/.test(cell)) || cells.find(cell => /^\d+$/.test(cell)); if (!poleCell) return null; const poleNumber = poleCell.match(/\d+/)?.[0]; if (!poleNumber) return null; const length = cells.find(cell => /\d+(?:\.\d+)?\s*[mｍ](?:\s|$)/i.test(cell)) || ''; const dateMatches = cells.join(' ').match(/(?:20\d{2}[\/\-]\d{1,2}[\/\-]\d{1,2}|\d{1,2}[\/\-]\d{1,2}|\d{1,2}月\d{1,2}日)/g) || []; const count = cells.find(cell => /\d+\s*(?:本|株)/.test(cell)) || ''; const content = cells.filter(cell => cell !== poleCell && cell !== length && !/^総面積|^栽培面積|^\d+(?:\.\d+)?\s*(?:㎡|[mｍ])$/i.test(cell) && !/^(?:列|ポール|長さ|本数|株数|株間|畝|マルチ|作業)/.test(cell)).map(cell => cell.replace(/20\d{2}[\/\-]\d{1,2}[\/\-]\d{1,2}|\d{1,2}[\/\-]\d{1,2}|\d{1,2}月\d{1,2}日/g, '').trim()).filter(Boolean).join(' / '); return { pole: `ポール↑${poleNumber}`, length, crop: content || '〃', planting: dateMatches[0] ? normalizeDateValue(dateMatches[0]) : '〃', count, spacing: cells.find(cell => /株間|cm/i.test(cell)) || '', bed: cells.find(cell => /畝/.test(cell)) || '', mulch: cells.find(cell => /マルチ/.test(cell)) || '', work: cells.find(cell => /作業|定植|誘引|除草|収穫/.test(cell)) || '' }; }
...
function extractLayoutPoleData(rows) { const headers = []; rows.forEach((row, rowIndex) => row.forEach((cell, columnIndex) => { const id = normalizeWorkbookFieldId(cell); if (workbookFieldIdSet.has(id)) headers.push({ id, rowIndex, columnIndex }); })); const extracted = new Map(); headers.forEach(header => { const nextColumn = headers.filter(item => item.rowIndex === header.rowIndex && item.columnIndex > header.columnIndex).map(item => item.columnIndex).sort((left, right) => left - right)[0] ?? Infinity; const nextRow = headers.filter(item => item.rowIndex > header.rowIndex && item.columnIndex >= header.columnIndex - 2 && item.columnIndex < nextColumn).map(item => item.rowIndex).sort((top, bottom) => top - bottom)[0] ?? rows.length; for (let rowIndex = header.rowIndex + 1; rowIndex < nextRow; rowIndex += 1) { const pole = extractWorkbookPoleRow(rows[rowIndex].slice(header.columnIndex, nextColumn)); if (!pole || pole.crop === '〃') continue; if (!extracted.has(header.id)) extracted.set(header.id, []); extracted.get(header.id).push(pole); } }); return extracted; }
