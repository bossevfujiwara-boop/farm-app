...
function loadFields() { try { const stored = JSON.parse(localStorage.getItem('farmnote-fields-v13-2026-27')); return applyLatestOverrides(stored && stored.length >= defaultFields.length ? stored.map(field => ({ ...field, id: normalizeFieldId(field.id) })) : structuredClone(defaultFields)); } catch { return applyLatestOverrides(structuredClone(defaultFields)); } }
function saveFields() { localStorage.setItem('farmnote-fields-v13-2026-27', JSON.stringify(fields)); document.getElementById('last-updated').textContent = '今 保存済み'; }
...
function applyWorkDetailCsv(rows) { const headerIndex = rows.findIndex(row => row.filter(cell => /^20\d{2}[\/\-]\d{1,2}[\/\-]\d{1,2}$/.test(cell)).length > 0); if (headerIndex < 0) return; const headers = rows[headerIndex]; const dateColumns = headers.map((value, index) => /^20\d{2}[\/\-]\d{1,2}[\/\-]\d{1,2}$/.test(value) ? { index, date: normalizeDateValue(value) } : null).filter(Boolean); const firstDateColumn = dateColumns[0].index; const groups = new Map(); let activeFieldId = ''; rows.slice(headerIndex + 1).forEach(row => { const fieldId = workbookRowFieldId(row) || normalizeWorkbookFieldId(row[0]); if (workbookFieldIdSet.has(fieldId)) activeFieldId = fieldId; if (!activeFieldId) return; const taskName = row.slice(0, firstDateColumn).map(value => String(value || '').trim()).find(value => value && !workbookFieldIdSet.has(normalizeWorkbookFieldId(value))) || ''; if (!taskName) return; dateColumns.forEach(({ index, date }) => { const checked = /^true$/i.test(String(row[index] || '').trim()); const unchecked = /^false$/i.test(String(row[index] || '').trim()); if (!checked && !unchecked) return; if (!groups.has(activeFieldId)) groups.set(activeFieldId, []); groups.get(activeFieldId).push({ name: taskName, date, checked }); }); }); groups.forEach((tasks, id) => { const field = fields.find(item => item.id === id); if (!field) return; field.workProcess = { poleGroups: [{ range: '作業詳細', area: '', crop: field.crop, variety: field.variety, tasks }] }; const completed = tasks.filter(task => task.checked).sort((left, right) => left.date.localeCompare(right.date)); field.progress = tasks.length ? Math.round(completed.length / tasks.length * 100) : 0; if (!completed.length) return; const latest = completed[completed.length - 1]; field.work = `完了：${latest.name}（${formatDate(latest.date)}）`; if (/片付け|撤去|終了/.test(latest.name)) field.status = 'empty'; else if (/収穫/.test(latest.name)) field.status = 'soon'; else field.status = 'growing'; }); saveFields(); renderMap(); renderDetail(); renderCalendar(); }
...
function renderWorkProcess(field) {
  ...
  document.getElementById('work-process-list').innerHTML = groups.length ? groups.map(group => `<div class="work-pole-group"><div class="work-pole-header"><strong>${group.range || 'ポール範囲未設定'}</strong><span>${group.area || '面積未設定'}</span></div><div class="work-pole-crop">${group.crop || '作物未設定'}${group.variety ? ` ${group.variety}` : ''}</div><ul class="work-checklist">${group.tasks.map(task => `<li class="${task.checked === false ? '' : 'done'}"><span>${task.name}</span><span>${task.date ? formatDate(task.date) : '未定'}</span></li>`).join('')}</ul>${group.memo ? `<p class="work-memo">${group.memo}</p>` : ''}<div class="work-temp-row"><span>積算温度：${group.accumulatedTemp || '未設定'}</span><span>適正生育温度：${group.idealTemp || '未設定'}</span></div></div>`).join('') : '<div class="column-empty">作業工程データ未登録<br><small>Excelの「2026-27作業詳細」シートを読み込むと表示されます。</small></div>';
}
