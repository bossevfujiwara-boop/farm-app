...
const workbookFieldIdSet = new Set(['a', 'b', 'c', 'd', 'e1', 'e2', 'f', 'gout', 'g1', 'g2', 'g3', 'g4', 'h', 'i', 'j', 'k', 'l1', 'l2', 'm', 'n', 'o']);
...
function workbookRowFieldId(row) { const cells = row.map(cell => String(cell ?? '').trim()).filter(Boolean); for (const cell of cells) { const labeled = cell.match(/^\s*([a-o](?:[1-6])?|gout)\s*(?:区画|圃場)\s*$/i) || cell.match(/(?:^|\s)([a-o](?:[1-6])?|gout)\s*(?:区画|圃場)/i); const id = labeled ? normalizeWorkbookFieldId(labeled[1]) : ''; if (workbookFieldIdSet.has(id)) return id; } return ''; }
