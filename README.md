...
- チェックボックス形式の作業詳細CSVは `work-detail-master-2026-27.csv` として配置します。日付を列見出し（例：`2026/09/20`）、各作業を行にし、`TRUE` を完了、`FALSE` を未完了として読み込みます。
- このCSVを差し替えると、全区画の作業工程、直近完了作業、進捗率、収穫工程に連動するステータスが起動時に更新されます。
- 日付セルには `TRUE: 牛糞・大谷石混ぜた` のようにチェック状態と日別メモを併記できます。`メモ`、`コメント`、`備考`列のテキストも作業履歴へ表示されます。
